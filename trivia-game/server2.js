...
app.get('/MichiganTrivia', function (req, res) {
    res.sendfile(__dirname + '/index.html');
});


var nextQuestionDelayMs = 2000; //5secs // how long are players 'warned' next question is coming
...
io.sockets.on('connection', function (socket) {
    ...
    socket.on('disconnect', function(zata) {
...
//	    if (players.getPlayerPoints(players.winningSocket.id) >= 10) {
...
//		io.sockets.emit('disconnect',true);
		...
		//console.log("there are " + players.playerCount + " players");
		//for (var player in players) {
		//skt.leave();
		//var pname = players.getPlayerName(player.playerId);
		//var pname = player.name;
		//console.log('SOCKET.IO player disconnect: '+ pname + ' for socket '+ player.playerId);
		//if (!pname) {
		    // already disconnected
		   // return;
		//}
